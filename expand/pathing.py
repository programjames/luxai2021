...
class Pather(object):
    def __init__(self, game, player, opponent):
        ...
        for u in opponent.units:
            self.is_open[u.pos.y][u.pos.x] = False
        ...
        while len(open_set) > 0:
            ...
            for x, y in open_set:
                for dx, dy in DELTA_NEIGHBORS:
                    ...
                    self.city_moves[new_y][new_x] = self.city_moves[y][x] + \
                        turn_delay(game.map.get_cell(new_x, new_y).road)
            open_set = new_open

        # print(*resource_types, file=sys.stderr)

    def city_distance(self, x, y):
        return self.city_moves[y][x]
    ...
    def sorted_city_moves(self, unit):
        ...
        for dx, dy in DELTA_NEIGHBORS:
            ...
            if self.can_move(new_x, new_y) and self.city_moves[new_y][new_x] < ds[0]:
                ds.insert(0, self.city_moves[new_y][new_x])
                dirs.insert(0, DIRECTION_FROM_DELTA[(dx, dy)])
