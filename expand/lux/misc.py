...
def turn_delay(road):
    # This is definitely not exact for all units and times.
    r = gc["PARAMETERS"]["UNIT_ACTION_COOLDOWN"]["WORKER"]
    return max(1, r * 2 - road)
...
def turns_till_day(turn):
    r = turn % (gc["PARAMETERS"]["DAY_LENGTH"] +
...
def turns_till_night(turn):
    r = turn % (gc["PARAMETERS"]["DAY_LENGTH"] +
                gc["PARAMETERS"]["NIGHT_LENGTH"])
    if r >= gc["PARAMETERS"]["DAY_LENGTH"]:
        return 0
    return gc["PARAMETERS"]["DAY_LENGTH"] - r
